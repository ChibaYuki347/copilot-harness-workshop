...
2026-05-26 で Foundry walkthrough をやったチームなら、これは「では Copilot から
使いましょう」の続編。デモされた Foundry 機能 (Content Understanding、PII、
translator) と同じ API。違いは Copilot が curl 例ではなく MCP で typed アクセス
する点。
