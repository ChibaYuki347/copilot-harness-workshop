...
「全部やる」1 つのエージェントは気が散ります。**メイン + 専門家** に分けると、
各役割にきれいなシステムプロンプト、より絞られたツール allowlist、独自のコンテキスト
予算を与えられます。1 ターンを委譲に使う → 専門家が 1 つのサマリを返す → メイン
セッションは focus を保つ。

## 代替手段 — カスタムエージェントを *使わない* 判断

- 「専門家」が **ステートレスで単一コマンドとして再利用** → [プロンプトファイル](prompt-files.md)。
  カスタムエージェントは ペルソナ / ツール / モデルの差別化があるときだけ overhead に
  見合う
- 作業が **いつもメインエージェントが inline でやる** → 会話を分断しない。カスタム
  エージェントは委譲のためで、再編成のためではない
- **共有スクリプトを持つ手順** だが同じエージェントが実行 → [Skill](skills.md)。
  エージェントとスキルは組み合わせ良し。別 identity が要らないなら skill の方が安い
