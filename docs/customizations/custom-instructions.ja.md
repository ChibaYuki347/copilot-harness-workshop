...
エージェントはコンテキストウィンドウにあるものしか知りません。規約、ビルドコマンド、
チームルール — どれもコード自体には書かれていないので、モデルは推測できません。
インストラクションは、そのギャップを **セッション開始時に一度** 埋める常時 prelude
です。毎ターン説明し直さなくて済むようになります。

## 代替手段 — インストラクションを *使わない* 判断

- 伝えたいのが **手順** → [Skill](skills.md) を選ぶ。インストラクションは
  200 行を超えると効きが落ちます
- 伝えたいのが **時々再利用するワンショットプロンプト** → [プロンプトファイル](prompt-files.md)。
  インストラクションは *毎ターン* 効くので、時々しか要らないものには重い
- 伝えたいのが **エージェントが破ってはいけないルール** → インストラクションは
  *第一線* だが、機械的な強制のために [hook](hooks.md) と組み合わせる
