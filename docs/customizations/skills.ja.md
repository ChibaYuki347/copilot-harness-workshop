...
ある種の作業は **手順** です: フェーズ、補助スクリプト、出力テンプレート、比較する
ためのルーブリック。インストラクションやプロンプトファイルでも手順を散文で書けますが、
補助ファイルを *バンドル* することはできません。Skill はその手順を **スクリプトと
一緒に** 出荷するフォルダ — エージェントは毎回 re-invent しなくて済みます。

## 代替手段 — Skill を *使わない* 判断

- 手順が **1 つの Markdown body、補助なし** → [プロンプトファイル](prompt-files.md)
  の方が軽量、スラッシュコマンドとして surface する
- 手順が **モデルが typed 引数で直接呼ぶツール** → [MCP ツール](mcp.md) として
  公開する
- 手順が **ユーザーがタイプせずに実行** → [hook](hooks.md) が自動発火。Skill は
  エージェントが使うことを選ぶ必要があります
